...
class RegistrationTests {
	...
	void testUserRegistration_TestCaseID001() throws Exception {
		...
		WebElement username = driver.findElement(By.id("registerusername"));//tìm biến đăng ký usename
		WebElement email = driver.findElement(By.id("registeremail"));//tìm biến đăng ký email
		WebElement password = driver.findElement(By.id("registerpassword"));//tìm biến đăng ký password
		WebElement gender = driver.findElement(By.id("male"));//tìm biến đăng ký giới tính

		WebElement registerbutton = driver.findElement(By.id("btn-register"));// tìm nút đăng ký
		// sau khi tìm các biến xong sẽ tìm giá trị
		username.sendKeys("");//usename
		email.sendKeys("");//email
		password.sendKeys("");//password
		gender.click();//giới tính chọn phương thức click
		registerbutton.click();// click nút đăng ký
		Thread.sleep(2000);// thời gian chạy là 2000 mili giây

		String actualString = driver.findElement(By.id("regunsuccessfulmsg")).getText();// tìm những gì mình nhập vào nếu để rỗng
		//nhập không đúng yêu cầu thì sẽ trả về
		assertTrue(actualString.contains("Registration unsuccessful"));//ở đây là không thành công
